...
//    protected string $model = Brand::class;
    ...
    public function index(Request $request)
    {
        ...
        $perPage = $request->input('per_page', 20); // Giới hạn số bản ghi trên mỗi trang
        if ($sortBy === 'is_active') {
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'file_logo' => 'nullable|image|mimes:jpeg,png,jpg,gif',
        ], [
            'name.required' => 'Vui lòng nhập hợp lệ!',
            'name.max' => 'Vui lòng nhập hợp lệ!',
            'file_logo.image' => 'Tệp phải là định dạng hình ảnh!',
            'file_logo.mimes' => 'Định dạng hình ảnh phải là jpeg, png, jpg, hoặc gif!',
        ]);
        // Lưu ảnh file_logo nếu có
        if ($request->hasFile('file_logo')) {
            $file = $request->file('file_logo');
            $path = $file->store('brands', 'public');
            $file_logo = Storage::url($path);
        }

        $brand = new EntitiesBrand();
        $brand->name = $validated['name'];
        $brand->file_logo = $file_logo;
        ...
        return redirect()->route('admin.brands.index')->with('success', 'Dữ liệu đã thêm thành công!');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('admin.brands.index')->with('success', 'Sửa dữ liệu thành công!');
    }
    ...
    public function bulkDelete(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('admin.brands.index')->with('success', 'Xoá dữ liệu thành công!');
        } catch (\Exception $e) {
            // Xử lý lỗi
            return redirect()->route('admin.brands.index');
        }
...
{
    $validated = $request->validate([
        'file_logo' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
    ]);

    $brand = EntitiesBrand::find($id);
    if ($brand && $request->hasFile('file_logo')) {
        // Xóa logo cũ nếu có
        if ($brand->file_logo && file_exists(public_path('logos/' . $brand->file_logo))) {
            unlink(public_path('logos/' . $brand->file_logo));
        }
        // Cập nhật logo mới
        $filePath = $request->file('file_logo')->store('logos', 'public');
        $brand->file_logo = basename($filePath);
        $brand->save();

        return response()->json(['success' => true, 'message' => 'Logo đã được cập nhật thành công!']);
    }
